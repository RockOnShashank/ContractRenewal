...
public class HistoryAdapter extends RecyclerView.Adapter<HistoryAdapter.ViewHolder> {
    ...
    HistoryAdapter(Context context, ArrayList<ServiceRequest> list) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (viewRequests.get(position).getStatus().equalsIgnoreCase("Canceled")){
            ...
            try {
                holder.tv_preferred_dateandtime.setText(getMonth(viewRequests.get(position).getCompletionDate()).substring(0,11).replaceAll("-"," "));
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
        if (viewRequests.get(position).getStatus().equalsIgnoreCase("Closed")){
            ...
            holder.tv_request_status.setTextColor(ctx.getResources().getColor(R.color.light_text_color));
            try {
                holder.tv_preferred_dateandtime.setText(getMonth(viewRequests.get(position).getCompletionDate()).substring(0,11).replaceAll("-"," "));
            } catch (ParseException e) {
                e.printStackTrace();
